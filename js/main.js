...
$(document).ready(function($) {
    ...
    moment.lang(mirror.config.lang);

    if(mirror.config.enableDishWasher){
    ...
    (function checkVersion() {
        if(mirror.config.checkVersion === 1){
            ...
            for (var i = 0; i <= events.length; i++) {
                ...
                if (seconds >= 0) {
                    if ((seconds <= 60 * 60 * 5) || (seconds >= 60 * 60 * 24 *2)) {
                        time_string = moment(startDate).fromNow();
                    }else {
                        time_string = moment(startDate).calendar();
                    }
    ...
    (function updateCalendar()
    {
        ...
        for (var i = 0; i < eventList.length; i++) {
            ...
            if(eventCount === (mirror.config.eventsToDisplay - 1)) {
                return;
            }
    ...
    (function updateCompliment()
    {
        while (compliment === lastCompliment) {
            compliment = Math.floor(Math.random () * mirror.config.compliments.length);
        }

        $('.compliment').updateWithText(mirror.config.compliments[compliment], 4000);
    ...
    (function updateCurrentWeather()
    {
        ...
        $.getJSON('http://api.openweathermap.org/data/2.5/weather', mirror.config.weatherParams, function(json) {
    ...
    {
            $.getJSON('http://api.openweathermap.org/data/2.5/forecast', mirror.config.weatherParams, function(json, textStatus) {
